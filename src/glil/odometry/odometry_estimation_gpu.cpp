...
gtsam::NonlinearFactorGraph OdometryEstimationGPU::create_factors(const int current, const std::shared_ptr<gtsam::ImuFactor>& imu_factor, gtsam::Values& new_values) {
  ...
  const auto create_binary_factor = [this](
                                      ...
                                      const glil::EstimationFrame::ConstPtr& target,
                                      const glil::EstimationFrame::ConstPtr& source) {
    auto stream_buffer = stream_buffer_roundrobin->get_stream_buffer();
    const auto& stream = stream_buffer.first;
    const auto& buffer = stream_buffer.second;

    for (const auto& voxelmap : target->voxelmaps) {
      auto factor = std::make_shared<gtsam_points::IntegratedVGICPFactorGPU>(target_key, source_key, voxelmap, source->frame, stream, buffer);
  ...
  const auto create_unary_factor = [this](
    ...
    for (const auto& voxelmap : target->voxelmaps) {
      auto factor = std::make_shared<gtsam_points::IntegratedVGICPFactorGPU>(fixed_target_pose, source_key, voxelmap, source->frame, stream, buffer);
      factor->set_enable_surface_validation(true);
      factors.add(factor);
    }
  };
