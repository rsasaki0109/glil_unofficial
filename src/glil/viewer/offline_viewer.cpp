#include <glil/viewer/offline_viewer.hpp>
...
void OfflineViewer::main_menu() {
  ...
  if (open_result) {
    if (!(*open_result)) {
      ...
      async_global_mapping.reset(new glil::AsyncGlobalMapping(*open_result, 1e6));
    }
  }
...
std::shared_ptr<GlobalMapping> OfflineViewer::load_map(guik::ProgressInterface& progress, const std::string& path) {
  ...
  std::shared_ptr<glil::GlobalMapping> global_mapping(new glil::GlobalMapping(params));
