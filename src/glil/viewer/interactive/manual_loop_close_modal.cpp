...
std::shared_ptr<Eigen::Isometry3d> ManualLoopCloseModal::align(guik::ProgressInterface& progress) {
  ...
  auto factor = std::make_shared<gtsam_points::IntegratedGICPFactor>(0, 1, target, source);
...
gtsam::NonlinearFactor::shared_ptr ManualLoopCloseModal::create_factor() {
  ...
  return std::make_shared<gtsam::BetweenFactor<gtsam::Pose3>>(target_key, source_key, relative, gtsam::noiseModel::Gaussian::Information(information_scale * H));
}
