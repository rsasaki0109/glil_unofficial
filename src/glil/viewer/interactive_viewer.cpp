#include <glil/viewer/interactive_viewer.hpp>

#include <atomic>
#include <thread>
#include <spdlog/spdlog.h>
#include <glil/mapping/sub_map.hpp>
...
#include <glil/util/trajectory_manager.hpp>

#include <glil/viewer/interactive/manual_loop_close_modal.hpp>
#include <glil/viewer/interactive/bundle_adjustment_modal.hpp>
...
InteractiveViewer::InteractiveViewer() : logger(create_module_logger("viewer")) {
  glil::Config config(glil::GlobalConfig::get_config_path("config_viewer"));
...
void InteractiveViewer::viewer_loop() {
  ...
  viewer->register_ui_callback("logging", guik::create_logger_ui(glil::get_ringbuffer_sink(), 0.5));
...
extern "C" glil::ExtensionModule* create_extension_module() {
  return new glil::InteractiveViewer();
}