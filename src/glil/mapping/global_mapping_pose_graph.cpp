...
#pragma omp parallel for num_threads(params.num_threads) schedule(dynamic)
    for (int i = 0; i < candidates.size(); i++) {
      ...
      if (params.registration_type == "GICP") {
        auto factor =
          std::make_shared<gtsam_points::IntegratedGICPFactor>(gtsam::Pose3(), 0, candidate.target->submap->frame, candidate.source->subsampled, candidate.target->tree);
        ...
        auto factor = std::make_shared<gtsam_points::IntegratedVGICPFactor>(gtsam::Pose3(), 0, candidate.target->voxels, candidate.source->subsampled);
    ...
    for (int i = 0; i < candidates.size(); i++) {
      ...
      factors.emplace_back(
        std::make_shared<gtsam::BetweenFactor<gtsam::Pose3>>(X(candidates[i].target->submap->id), X(candidates[i].source->submap->id), T_target_source[i], noise_model));
    }
